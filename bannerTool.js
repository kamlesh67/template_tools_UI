...
<% _.forEach(banners, function(item) { %>
  ...
  <img src="<%= item.img %>" style="max-height: 100%;min-height: 100px;width: 100%;" />
  </div>
<% }); %>
`);
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <button class="close" id="modalCloseBtnBanner">&times;</button>
        </div>
...
const toolTemplate = function (values, isViewer = false) {
 ...
 {
    return `<div style="position:relative;background-color:#F6F8F8;border:1px solid rgba(0,0,0,.125);border-radius:4px;margin:auto;text-align:center; padding:20px 10px;">
  ...
  <p style="font-size:13px;color:#808080;hyphens: auto;">Click here to select a community banner from the list.</p>
  </div>
    ${isViewer ? modalTemplate({ banners: values.data.banners }) : ''}`;
}
};
        ...
        mount(node, value, updateValue, data) {
            ...
            addButton.onclick = function () {
                ...
                setTimeout(() => {
                    ...
                    const closeBtn = document.querySelector('#modalCloseBtnBanner');
