(function () {
  ...
  <% _.forEach(speakers, function(item) { %>
    ...
    <div class="speakers-media"> <img src="<%= item.profile_img %>" alt="image" style="height:auto; width: 100%; object-fit:cover; border-radius: 8px;" /> </div>
      <h3 style="style="margin:5px 8px 0; font-size:13px; color: ${
        values.speakerTitleColor
      };overflow: hidden;  display: block;  text-overflow: ellipsis;  white-space: nowrap;"><%= item?.name %> </h4>
      <h4 style="margin:5px 8px 0; font-size:12px; height: 35px; overflow:hidden; color: ${
        values.speakerDesignationCompanyColor
      };"><%= item?.designation %> <%= item?.designation && item?.company ? ',' : '' %> <%= item?.company %> </h5>
    </div>
  <% }); %>
`);
