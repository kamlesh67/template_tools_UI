(function () {
  ...
  <% _.forEach(speakers, function(item) { %>
    ...
    <div class="speakers-media"> <img src="<%= item.profile_img %>" alt="image" style="height:auto; width: 100%; object-fit:cover; border-radius: 8px;" /> </div>
      <h3 style="margin:5px 10px 0; text-align: left; color: ${theme.primary};overflow: hidden;  display: block;  text-overflow: ellipsis;  white-space: nowrap;"><%= item?.name %> </h4>
      <h5 style="margin:5px 10px 0; text-align: left;color: ${theme.secondary};"><%= item?.designation %> <%= item?.designation && item?.company ? ',' : '' %> <%= item?.company %> </h5>
    </div>
  <% }); %>
`);
