...
<% _.forEach(logos, function(item) { %>
  ...
  <img src="<%= item.img %>" style="height: auto;width: 100%; padding:15px;" />
  </div>
<% }); %>
`);
...
const toolEmailTemplate = function (values, isViewer = false) {
  ...
  {  return `<div logoId="${values?.logoLibrary?.selected?.id}" 
      style="position:relative;display:block; background-color:#fff;border:1px solid rgba(0,0,0,.125);border-radius:4px;margin:auto;text-align:center;width:auto;margin-bottom: 0px;height: auto;padding: 14px 10px;display: flex;">
