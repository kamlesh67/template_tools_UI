(function () {
  ...
  const speakerList = function (speakers, sessionId) {
    ...
    if (speakers.length > 4) {
      ...
      };height: 30px;width: 30px;overflow: hidden;border-radius: 30px; display: flex; justify-content: center;align-items: center;font-size: 13px; line-height: 30px; display: block;">
              +${speakers.length - 4}
            </div>`;
    }
    return sps;
  ...
  const boothList = function (sponsors, sessionId) {
    ...
    if (sponsors.length > 2) {
      bts += `<div id="remainingSessionSponsorCount" style="background-color:${
        ...
        theme.secondaryFontColor
      };height: 30px;width: 30px;overflow: hidden;border-radius: 4px; display: flex; justify-content: center;align-items: center;font-size: 13px; line-height: 30px; display: block;"> 
               +${sponsors.length - 2}
            </div>`;
    }
    return bts;
